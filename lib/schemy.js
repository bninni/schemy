...
var argNamesMap = {
	...
	JSONModule = new function(){
		//to get the 'global' object. returns empty object otherwise
		function getGlobal(){
			try{
				return window;
			}
		...
		*****/
		function fromSchemyJSON( value, data ){
			
			var obj = value._Schemy,
				global = getGlobal(),
				data, con;
			
			if( !is( obj, Object ) || !exists(global) ) return {};
			
			if( !is( obj.type, String ) ) return {};
			
			con = global[ obj.type ];
			
			if( !is( con, Function ) ) return {};
			...
			switch( con ){
				...
				case Function:
					//if its a global function, then return that
					if( is( data, String ) && exists( global[data]) ) return global[data];
					else if( is( data, Array ) ) return Function.apply( Function, data );
					return new Function();
			}
			
			if( !exists( data ) ) return new con();
		...
		*****/
		function decodeValue( value, addCircular, parentObj, parentKey ){
			...
			if( value._SchemyUndef === true ) return undefined;
			
			//if it is a reference to another object, add to the queue
			if( is( value._SchemyRef, Array ) ) return addCircular( parentObj, parentKey, value._SchemyRef );
			
			//create the new based object based off of the _Schemy object (if it exists)
			newObj = fromSchemyJSON( value );
			
			forEach( value, function( value, key ){
				if( key === "_Schemy" ) return;
		...
		*****/
		function toSchemyJSON( value ){
			...
			if( value instanceof Error ){
				...
				case Function:
					data = funcName( value );
					//if its a global function, then the data is the name of the function
					if( exists( global ) && global[data] === value ){
						newObj.data = data;
					}
					else{
		...
		*****/
		function encodeValue( value, path, objs, paths ){
			...
			if( is( value, undefined ) ) return { _SchemyUndef : true }
			
			index = objs.indexOf( value );
			
			//if it is a circular reference, then create the reference object
			if( index > -1 ) return { _SchemyRef : paths[ index ] };
