...
JSON
	- what if, when checking for ('value' in obj[key]),the object has the value property but is not a PropertyDescriptor ???
	- in addProperty, what if 'get' and 'set' are circular?
	- ignore 'message' from Error (it is included in 'data')
	- remove all default values from Object.getOwnPropertyDescriptor
...
var argNamesMap = {
	...
	JSONModule = new function(){
		...
		//when getting a property description, there are some exceptions to the default methods
		PropertyDescriptors = new function(){
			...
			this.get = function( obj, key ){
				...
				//if all the default values are true, then it is simply an assignment
				if( valueOnly && 'value' in desc ) return desc.value;
			...
			//The stack getter and setters are native functions, just replace the current value
		...
		*****/
		function fromSchemyJSON( type, data ){
			...
			var constructor, index;
				
			if( !is( type, String ) ) return {};
			
			index = GlobalMap.keys.indexOf( type )
			if( index === -1 ) return {};
				
			constructor = GlobalMap.values[ index ];
		...
		*****/
		...
		function goToPath( path, obj ){
			path.forEach(function( key ){
				//if it is a circular reference object, then the path is unresolved so return undefined
				if( obj === circularReferenceObj || ('value' in obj && obj.value === circularReferenceObj) ) return obj = circularReferenceObj;
		...
		To decode a value
		*****/
		function decodeValue( value, addCircular, addProperty, decodeCircularType, parentObj, parentKey ){
			...
			if( is( _Schemy, String ) ){
				switch( _Schemy ){
					...
					case "Infinity":
						return Infinity;
					default:
						return GlobalMap.global[_Schemy];
				}
			}
			...
			//if it is a number, then it can be assigned using Object.defineProperty
			if( is( _Schemy, Number ) ){
				PropertyDescriptors.set( value );
				addProperty( parentObj, parentKey );
			}
			
			//if its an object, then determine the base for the newObj
			if( is( _Schemy, Object ) ){
				type = _Schemy.type;
				//if the type is a reference to another object, add it to the queue
				if( is( type, Array ) ) return decodeCircularType( value, type, parentObj, parentKey );
				else newObj = fromSchemyJSON( type, _Schemy.data );
			}
			else newObj = {};
			
			createNewObj( value, newObj, addCircular, addProperty, decodeCircularType );
				
			return newObj;
		}
		
		function createNewObj( oldObj, newObj, addCircular, addProperty, decodeCircularType ){
			
			delete oldObj._Schemy;
			
			forEach( oldObj, function( value, key ){
				newObj[key] = decodeValue( value, addCircular, addProperty, decodeCircularType, newObj, key );
			});
		...
		function decode( value ){
			...
			function addCircular( obj, key, path ){
				circular.push(function(){
					...
					//if it points to circularReferenceObj, then it points to an object that wasnt created yet, so add to end of list
					if( newObj === circularReferenceObj ) addCircular( obj, key, path )
					else obj[key] = newObj;
				});
				return circularReferenceObj;
			}
			
			//to add a property to an object
			function addProperty( obj, key ){
				properties.push(function(){					
					//If the 'value' of the property is the circularReferenceObj, then add to end of the array
					if( obj !== circularReferenceObj && 'value' in obj[key] && obj[key].value !== circularReferenceObj ) Object.defineProperty( obj, key, obj[key] );
					else( addProperty( obj, key ) );
				});
			}
			
			//to decode a type that is circular
			function decodeCircularType( value, path, obj, key ){
				circularTypes.push(function(){
					var newObj,
						data = value._Schemy.data,
						newType = goToPath( path, retObj );
						
					//if it points to circularReferenceObj, then it points to an object that wasn't created yet, so add to end of list
					if( newType === circularReferenceObj ) return decodeCircularType( value, path, obj, key )
			...
			retObj = decodeValue( value, addCircular, addProperty, decodeCircularType, true );
			
			while( properties.length || circular.length || circularTypes.length ){
				//process all of the properties that are not circular references
				runAndClear(properties);
				//fill in all of the circular references
				runAndClear(circular);
				//process objects that have types that are circular references
				runAndClear(circularTypes);
			}
		...
		*****/
		function toSchemyJSON( value, objs, paths ){
			var data, typeName, index,
				constructor = type(value),
				newObj = {},
				index = GlobalMap.values.indexOf( constructor );
				
			
			//if its a global value, the typename is the key
			if( index > -1 ) typeName = GlobalMap.keys[index];
			else{
		...
		*****/
		function encodeValue( value, path, objs, paths ){
			...
			var keys, enumKeys, typeKeys, newValue, index;
			...
			//get the keys to the original object and its constructor
			keys = Object.getOwnPropertyNames( value );
			enumKeys = Object.keys( value );
			
			typeKeys = Object.getOwnPropertyNames( type( value ) );
			typeKeys = typeKeys.concat( Object.getOwnPropertyNames( type(value).prototype ) );
			
			//encode the object
			newValue = {};
			
			keys.forEach(function( key ){
				var newObj, newPath, obj;
				
				//ignore keys that are non enumerable and also exist in the type keys
				if( enumKeys.indexOf( key ) === -1 && typeKeys.indexOf( key ) > -1 ) return;
				
				//get the property descriptor
				obj = PropertyDescriptors.get( value, key );
				
				newPath = path.slice();
				newPath.push(key);
				
				newValue[key] = encodeValue( obj, newPath, objs, paths );
			});
