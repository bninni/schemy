...
Give (ordered, sort, object) an option to clone the object before applying the schema
-same for 'apply'
...
Add function to handle array schema
...
Should 'after' run before or after the additionalSchemaCheck?
...
Test if Function.toString exists before running argNames??

See how many times buildSchema is called when it is not necessary
-use Schema object to know if it is a schema or not?
...
Object:
	every : schema every value should match
	specific : (schema for each specific key)
	removeExtra (to remove any additional keys)
	removeUndefined (to remove keys that have specific schemas but are undefined)
	addMissing (to add specific keys if they are not included in the original object
Array:
...
function clone( obj ){
	...
	if( !exists(obj) || typeof obj !== 'object') return obj;
